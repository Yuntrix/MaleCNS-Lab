...
# LOOMING
#
# Mevcut engineering stimulus.
# Raw retina simülasyonu değildir.
# ============================================================
...
# 2 Hz x 200 ms
#
# Bu biyolojik ölçüm değildir.
# Autonomous internal-drive modelidir.
# ============================================================
...
# ============================================================
# INTERNAL BURST ZAMANLAMASI
#
# Bunlar davranış komutu değildir.
# Sadece modeled internal neural drive zamanlarıdır.
# ============================================================
...
# BODY / WORLD MODEL
#
# Bunlar neural model değildir.
# Cartoon body / actuator / world mechanics katmanıdır.
# ============================================================

MOVE_HOLD_MS = 450.0

MIN_AIRBORNE_MS = 900.0

FLY_REFRESH_MS = 500.0
...
# Horizontal speeds are normalized screen units / second.

MOVE_TARGET_SPEED = 0.075

MOVE_ACCELERATION = 0.30

AIR_HORIZONTAL_ACCELERATION = 0.07

MAX_AIR_HORIZONTAL_SPEED = 0.13


# Vertical normalized screen units / second.

GRAVITY = 1.60

FLY_TAKEOFF_VELOCITY = -0.72

FLY_EXTRA_THRUST = -0.10

MAX_UPWARD_VELOCITY = -0.95
...
print("=" * 80)
print("MALECNS AUTONOMOUS OBS SERVER V3")
print("=" * 80)
...
def build_endogenous_population():
    ...
    # --------------------------------------------------------
    # CACHE YOKSA CONNECTOME'DAN YENİDEN SEÇ
    # --------------------------------------------------------

    print(
        "Endogenous population hazırlanıyor..."
    )
    ...
    # --------------------------------------------------------
    # Direct descending connectivity
    # --------------------------------------------------------
...
class BodyState:
    ...
    def __init__(
        ...
        self.airborne_until_ms = 0.0


        self.previous_raw_action = "IDLE"
    ...
    def update(
        ...
        # FLY EVENT
        #
        # FLY has priority over MOVE.
        #
        # A short neural FLY event produces a physical takeoff
        # impulse and minimum airborne period.
        #
        # Neural output is NOT modified.
        # ====================================================
        ...
        if raw_action == "FLY":
            ...
            if not self.airborne:
                ...
                # Continued neural flight signal
                self.state = "FLY"
        ...
        # MOVE
        #
        # Grounded ise yürüyüş actuator hold.
        #
        # Havada MOVE gelirse FLY state'ini kesmez.
        # ====================================================
        ...
        if not self.airborne:
            ...
            # Airborne inertia / small forward tendency
        ...
        if self.y >= GROUND_Y:
            ...
            # Minimum flight time dolmadan touchdown olursa
            # küçük bir actuator lift ile airborne kal.
            if (
...
def brain_loop():
    ...
    last_logged_raw_action = "IDLE"

    last_logged_body_state = "IDLE"


    print()

    print(
        "Brain + body loop başladı."
    )
    ...
    while True:
        ...
        if (
            ...
            raw_action != last_logged_raw_action

            or

            body.state != last_logged_body_state
            ...
            last_logged_raw_action = (
                raw_action
            )


            last_logged_body_state = (
                body.state
            )
...
print(
    "Body physics artık SERVER tarafında."
)

print(
    "FLY, MOVE tarafından havadayken kesilemez."
)
