...
#
# V6: screen LEFT/RIGHT IS mapped now (see below).
# ============================================================
...
# ============================================================
# SCREEN <-> ANATOMY MAPPING  (V6 FIX #1)
#
# The left optic lobe views the LEFT half of the screen.
# Set this to False if the capture / vision bridge is
# mirrored and the fly runs toward the stimulus instead of
# away from it.
# ============================================================

ANATOMICAL_LEFT_IS_SCREEN_LEFT = True

# Minimum gap between two stimulus-driven turns, so a burst
# of triggers cannot make the fly vibrate in place.
TURN_COOLDOWN_MS = 300.0
...
# How much of the existing momentum survives a turn.
TURN_MOMENTUM_KEEP = 0.35
...
print("ANATOMICAL LEFT / RIGHT -> SCREEN LEFT / RIGHT")
print("=" * 80)
...
# V6 only adds directional steering from the visual input.
# ============================================================
...
class BodyState:
    ...
    def __init__(self):
        ...
        self.turn_cooldown_until_ms = 0.0
    ...
    def turn_away_from(
        ...
        stimulus_side:
            -1  looming on the SCREEN LEFT
            +1  looming on the SCREEN RIGHT
             0  center / bilateral -> no turn

        The fly turns AWAY from the stimulus.
        Only the heading is set here; the actual movement
        (MOVE / FLY / JUMP) still comes from the brain, so
        the V4 actuator model is untouched.
        """
        ...
        if (
            brain_time_ms
            <
            self.turn_cooldown_until_ms
        ):
            return

        escape_facing = -stimulus_side

        if escape_facing != self.facing:

            self.facing = escape_facing

            # Bounce the existing momentum instead of
            # teleporting the velocity.
            self.vx *= -TURN_MOMENTUM_KEEP
    ...
    def update(
        ...
        if self.x <= LEFT_WALL:
            ...
            # A physical wall bounce wins over a visual turn.
            self.turn_cooldown_until_ms = (
                brain_time_ms
                +
                TURN_COOLDOWN_MS
...
def brain_loop():
    ...
    while True:
        ...
        # ====================================================
        # SCREEN MAPPING + ESCAPE HEADING   (V6 FIX #1)
        #
        # The anatomical pools are translated into SCREEN
        # sides here, and the fly turns away from the side
        # the looming stimulus came from.
        #
        # Edge triggered: a 60 ms stimulus spans 3 chunks but
        # only produces one turn.
        # ====================================================

        if ANATOMICAL_LEFT_IS_SCREEN_LEFT:

            screen_left_active = left_active
            screen_right_active = right_active

        else:

            screen_left_active = right_active
            screen_right_active = left_active
        ...
        if (
            ...
            stimulus_side = 0
            screen_visual_input = "SCREEN_CENTER"

        elif screen_left_active:
            stimulus_side = -1
            screen_visual_input = "SCREEN_LEFT"

        elif screen_right_active:
            stimulus_side = 1
            screen_visual_input = "SCREEN_RIGHT"
        ...
        if (
            stimulus_side is not None
            and
            stimulus_side != last_stimulus_side
        ):
            ...
            print(
                f"[ESCAPE] "
                f"{brain_time_ms:8.0f} ms "
                f"| {screen_visual_input:14s}"
                f"| facing={body.facing:+d}"
            )

        last_stimulus_side = stimulus_side
        ...
        if (
            ...
            print(
                ...
                f" | screen={screen_visual_input:14s}"
                f" | raw={raw_action:5s}"
...
// V6 FIX #2
// The artwork points LEFT by default: the head, the eyes and
// the antennae are drawn on the left side, the wings on the
// right. So facing = +1 (moving RIGHT) has to mirror the
// sprite. Before this, the fly walked backwards.
const SPRITE_FORWARD_X = -1;
...
function render() {
    ...
    const spriteScaleX = facing * SPRITE_FORWARD_X;
...
class OverlayHandler(
    ...
    def do_GET(self):
        ...
        # ====================================================
        # SCREEN LEFT VISUAL INPUT
        #
        # Routed to the anatomical pool through
        ...
        if path == "/trigger_right":
            ...
            self.set_visual_command(
                (
                    "looming_right_ms"
                    if ANATOMICAL_LEFT_IS_SCREEN_LEFT
                    else "looming_left_ms"
                ),
                "looming_screen_right"
            )
...
print()
print("SCREEN VISUAL TEST ROUTES:")
print(
...
print()
print(
    "Legacy /trigger -> center/bilateral"
)
...
print(
    "Sprite mirror fix aktif (SPRITE_FORWARD_X = -1)."
)
print("=" * 80)
